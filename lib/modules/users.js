...
function getUserFromList(redis, list, scope, callback){
	...
	multi.exec(function(err, replies) {
		...
		for (i=0;i<replies.length;i++)	{
			...
			if ((scope==0 && user.region<10) || (scope==1 && user.region<100) || (scope==2))
				users.push (user)
		}	
		callback(null, users);
	});	
...
exports.GetUser = function (redis, params, callback){
	...
	multi.hgetall ($(config.project_key,'user', id));
	multi.smembers($(config.project_key, 'user', id, 'tags'));
	multi.smembers($(config.project_key, 'user', id, 'cats'));
...
exports.GetUsers = function (redis, params, callback){	
	var scope = params.region_scope || 2;
...
exports.Search = function  (redis, params, callback){
	...
	search.query(query = params.q).end(function(err, ids){
		...
		getUserFromList(redis, list, 2, function return_users(err, users_found){ //todo scope
