...
function GetUsersIDListByScope (redis, params, callback){
	var keys = []
	...
	var key=$(config.project_key, 
			'cat', params.id_cat || 1,  //always cat
 	...
 	multi.zrangebyscore(key, 0, params.scope.region);

	multi.exec(function(err, replies) {
		var result = {}
		result.users = replies[0];
		callback (err, result);
	});
}
exports.GetUsersIDListByScope=GetUsersIDListByScope

function GetUsersByScope (redis, params, callback){
	GetUsersIDListByScope(redis, params, function (err, data){
		params.list = data.users;
		getUserFromList(redis, params, callback);
	})
}
exports.GetUsersByScope=GetUsersByScope
...
exports.GetUserByLinkedinId = function (redis, params, callback){
	...
	redis.get (linkedin_id, function (err, id){
		if (!err && id){
			...
			callback (err, null); //doesn't exist or eerro
		}
	});	
}
...
exports.SetUser = function (redis, params, callback){
	...
	function setUser(redis, params,callback){
		...
		if (params.existent_user)
			created = existent_user.creation_date || created;
		...
		if (params.existent_user){
			for (c=0;c<params.existent_user.cats.length;c++){
				multi.srem($(config.project_key, 'cat', params.cats[c], 'users'), user.id);
			}
					...
					for (var f=0;f<free_values.length;f++){
						for (var e=0;e<entre_values.length;e++){
							var key = $(config.project_key, 
								...
								'freelance' , free_values[f], 
								'entrepreneur', entre_values[e]);
							multi.zadd(key, user.region || 1000, user.id);
						}
					}
				}
		...
		if (params.existent_user){
			for (t=0;t<params.existent_user.tags.length;t++){
				multi.srem($(config.project_key, 'tag', params.tags[t], 'users'), user.id);
			}
		...
		//search indexes
		//region index
		//remove user from region if exists
		if (params.existent_user){
			for (r=0;r<config.regions.length;r++){
				multi.zrem($(config.project_key, 'users', 'region', r.value), user.id);
			}
		}
		multi.zadd($(config.project_key, 'users', 'region'), (user.region || 1000), user.id); // add user to region

		//freelance index
		if (params.existent_user)
			multi.zrem($(config.project_key, 'users', 'freelance'), user.id);

		var freelance = (user.other_data && user.other_data.freelance) ? 1 : 0 
		multi.zadd($(config.project_key, 'users', 'freelance'), freelance, user.id);

		//entrepreneur index
		if (params.existent_user)
			multi.zrem($(config.project_key, 'users', 'entrepreneur'), user.id);

		var entrepreneur = (user.other_data && user.other_data.entrepreneur) ? 1 : 0 
		multi.zadd($(config.project_key, 'users', 'entrepreneur'), entrepreneur, user.id); 
		...
		multi.exec(function(err, replies) {
			if (err)
			{
				...
				exports.GetUser(redis, {id:user.id}, function get_user (err, user_from_db){
					...
					{
						throw 'user id not found'
						console.log ('user ' + user.id + ' not recorded ok')
	...
	exports.GetUser(redis, params, function getTags(err, existent_user){
