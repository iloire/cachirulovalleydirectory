...
function PopulateUsers(callback){
	...
	module_users.AddUsers(redis, params, function (err, users_db){
		for (var i=0, l=users_db.length;i<l;i++){
			console.log ('populating user ' + users_db[i].name + ' ..')
		}
...
function AddExtraFakeUsers(callback){
	...
	for (i=0;i<extrausers;i++){
		...
		user.bio = "This is a fake profile. " + user.name;
		...
		user.location = 'spain'
		user.region = 100
		user.cats = [Math.ceil(cats.length * Math.random())];
